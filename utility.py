...
DEFAULT_DATA_SEASON = "20252026"
RAW_DATA_DIR = f"./data/raw_games/{DEFAULT_DATA_SEASON}/"
...
def fetch_and_vault_raw_data(game_id, raw_dir=None, throttle_seconds=0, max_retries=0):
    """
    ...
    file_path = os.path.join(target_dir, f"{game_id}.json")
    ...
    try:
        for attempt in range(max_retries + 1):
            ...
            if response.status_code == 429 and attempt < max_retries:
                ...
                print(f"Rate limited on raw game {game_id}. Waiting {wait_seconds}s before retrying.")
...
_ACTIVE_DATA_SEASON = DEFAULT_DATA_SEASON
MASTER_REPORT_FILE = os.path.join(BASE_DATA_DIR, f"master_report_{DEFAULT_DATA_SEASON}.json")


def set_active_data_season(season):
    """Set the default season used by dashboard reads."""
    global _ACTIVE_DATA_SEASON
    _ACTIVE_DATA_SEASON = str(season or DEFAULT_DATA_SEASON)


def get_active_data_season():
    return _ACTIVE_DATA_SEASON


def master_report_path(season=None):
    selected_season = str(season or _ACTIVE_DATA_SEASON or DEFAULT_DATA_SEASON)
    return os.path.join(BASE_DATA_DIR, f"master_report_{selected_season}.json")


def raw_data_dir(season=None):
    selected_season = str(season or _ACTIVE_DATA_SEASON or DEFAULT_DATA_SEASON)
    return os.path.join(RAW_DATA_BASE_DIR, selected_season)


def available_master_report_seasons():
    base_path = Path(BASE_DATA_DIR)
    if not base_path.exists():
        return []

    seasons = []
    for report_path in base_path.glob("master_report_*.json"):
        season = report_path.stem.replace("master_report_", "", 1)
        if season:
...
def load_master_reports(season=None):
    """Load the central master vault for all processed goalie reports."""
    report_path = master_report_path(season)
...
def save_master_reports(master, season=None):
...
def save_report_to_master(report, goalie_name, season=None):
    """Add a single report to the master file for fast lookup."""
    if not report:
        return None

    master = load_master_reports(season=season)
    ...
    save_master_reports(master, season=season)
    return master_report_path(season)
...
def load_local_report(game_id, goalie_name, season=None):
...
def _load_raw_game(game_id, season=None):
    raw_path = os.path.join(raw_data_dir(season), f"{game_id}.json")
...
def _raw_game_exists(game_id, season=None):
    return os.path.exists(os.path.join(raw_data_dir(season), f"{game_id}.json"))
...
def build_late_xga_collapse_proof():
    from slc_prediction_engine import build_late_xga_collapse_proof as _build_late_xga_collapse_proof

    return _build_late_xga_collapse_proof()
